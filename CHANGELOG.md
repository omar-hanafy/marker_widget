...
Stable successor to 2.0.1. This entry is the full delta from 2.0.1 and absorbs
the published `2.1.0-dev.1` prerelease (its section below remains as the
historical record). Migration guidance ships as the
`migrating-marker-widget-v2-to-v3` agent-plugin skill.

### Breaking

- Deterministic image readiness replaces delay-based waiting. `waitForImages`,
  `initialImageDelay`, and `imageRepaintDelay` are removed from the render
  options and the renderer. Declare the `ImageProvider`s a widget displays in
  `MarkerRenderOptions.imageDependencies`: each is decoded before capture
  (against the render environment's pixel ratio, locale, text direction, and
  asset bundle) and retained until the capture completes. A provider that
  fails to load throws the new `MarkerImageLoadException` instead of
  capturing a blank marker.
- Renamed `WidgetBitmapRenderOptions` to `MarkerRenderOptions`.
- Removed `buildMarkerCacheKey` and `buildClusterCacheKey` (their
  `extra?.toString()` comparison silently collided for objects with the
  default `toString`). Use the new `MarkerCacheKey` value object
  (`MarkerCacheKey(id, brightness: ..., locale: ..., extra: ...)` and
  `MarkerCacheKey.cluster(count: ...)`); it carries content inputs only,
  because the renderer keys every cache entry by the resolved logical size
  and pixel ratio itself. `extra` is compared with `==`.
- Removed the top-level `defaultMarkerIconRenderer`; the shared instance is
  now `MarkerIconRenderer.shared`.
- `MarkerIcon`'s constructor is no longer const: it defensively copies
  `bytes` and stores them as an unmodifiable view (writes throw
  `UnsupportedError`), and its content hash is computed once and memoized.
- `MapBitmapOptions` and `MarkerRenderOptions` no longer extend `Equatable`;
  the public `props` member is gone (structural `==`/`hashCode` are
  hand-written and unchanged in behavior) and the `equatable` dependency is
  removed from the package.
- Raised minimums: Dart `^3.12.0`, Flutter `>=3.44.0`,
...
- `MarkerIcon.toMarker` and the widget `toMarker` extension throw
  `ArgumentError` when the base is an `AdvancedMarker` (previously it flowed
  silently through the classic marker pipeline); use `toAdvancedMarker` or
  `toAdvancedPinMarker`.
- Renders are bounded by default: `maxConcurrentRenders` (default 3) gates
  how many off-screen render trees exist at once (FIFO), and
  `maxRasterPixels` (default 4194304, one 2048 x 2048 physical bitmap)
  rejects larger rasterizations with `ArgumentError`. Set either to null to
  disable.
- Dimensions and ratios are validated as positive and finite everywhere:
  ...
  `StateError` instead of propagating to the platform, and
  `MarkerIconRenderer` constructor configuration is validated at runtime
  (previously a debug-only assert covered `maxCacheEntries` only).
- Contextless rendering in a multi-view app with no implicit view now throws
  a `StateError` asking for a `BuildContext` instead of picking an arbitrary
  `FlutterView`.

### Added

- `MarkerRenderOptions.imageDependencies` and `MarkerImageLoadException`
  (the deterministic readiness contract above).
- `MarkerCacheKey` / `MarkerCacheKey.cluster` structured cache keys.
- `MarkerIconRenderer.shared`.
- `MarkerIconRenderer.maxConcurrentRenders` and
  `MarkerIconRenderer.maxRasterPixels` configuration.
- Re-exports of the remaining Google Maps types used by the package API:
  ...
  `GroundOverlay`, and `GroundOverlayId`. The whole marker flow now works
  from the `marker_widget` import alone.
- A debugging `MarkerIcon.toString` (size, pixel ratio, byte count).
...
- Screen geometry no longer leaks into rendered markers: safe-area padding,
  ...
  are still inherited.
- Image-dependency decoding runs before a concurrency slot is taken, so slow
  network decodes never starve the render gate.
